...
async function fetchHome() {
  ...
  const { data } = await http.get(`${KS}/home`);
  ...
  const latest = pickArray(
    ...
    .filter(Boolean);
  _homeCache = { trending, latest };
  // Drop the cache after a short window so subsequent navigations pick up
  // fresh edge-cached upstream responses.
  setTimeout(() => {
    _homeCache = null;
  }, 30_000);
  return _homeCache;
