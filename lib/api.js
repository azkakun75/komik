...
function fetchHome() {
  ...
  // caller can retry instead of being pinned to a rejected promise. The 30s
  // expiry guard checks identity so a stale timeout from an earlier (failed)
  // promise doesn't blow away a fresh in-flight one started by the retry.
  const p = _homePromise;
  p.catch(() => {
    if (_homePromise === p) _homePromise = null;
  }).finally(() => {
    setTimeout(() => {
      if (_homePromise === p) _homePromise = null;
    }, 30_000);
