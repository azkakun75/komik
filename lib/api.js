...
export async function getTrending() {
  const { data } = await http.get(`${API_PREFIX}/trending`);
...
export async function getPustaka(page = 1) {
  const { data } = await http.get(`${API_PREFIX}/pustaka/${page}`);
  return pickArray(data, "comics", "data").map((x) => normaliseComic(x, "library"));
}

export async function getRecommendations() {
  const { data } = await http.get(`${API_PREFIX}/recommendations`);
...
export async function getGenres() {
  const { data } = await http.get(`${API_PREFIX}/genres`);
...
export async function searchComics(query) {
  ...
  const { data } = await http.get(
    `${API_PREFIX}/search?q=${encodeURIComponent(query.trim())}`
  );
...
export async function getComicDetail(slugOrLink) {
  ...
  const { data } = await http.get(`${API_PREFIX}/comic/${cleanSlug}`);
  if (!data) return null;
  return {
...
export async function getChapter(chapterLink) {
  ...
  const { data } = await http.get(`${API_PREFIX}/chapter${path}`);
  return {
